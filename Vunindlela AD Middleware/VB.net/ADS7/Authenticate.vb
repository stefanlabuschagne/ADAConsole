...
Namespace ADS7
	Public Class Authenticate
		...
		Public Function getUserID(Persalid As String) As String
			Dim a As String
			If (InlineAssignHelper(a, Persalid.ToString().ToUpper().Substring(0, 1))) IsNot Nothing Then
				If a = "P" Then
		...
		Public Sub getADServer(ServerShortName As String)
			...
			Dim sqlDataReader As SqlDataReader = New SqlCommand() With { _
				...
				Key .CommandText = "SELECT * FROM VIP..SERVER_TRANSLATION WHERE SHORT_NAME='" & ServerShortName.ToUpper() & "'", _
				Key .CommandType = CommandType.Text _
			}.ExecuteReader()
			If sqlDataReader.HasRows Then
				sqlDataReader.Read()
				Me.IP_ADDRESS = sqlDataReader.GetValue(sqlDataReader.GetOrdinal("IP")).ToString()
				Me.DC = sqlDataReader.GetValue(sqlDataReader.GetOrdinal("DC")).ToString()
			Else
				Me.IP_ADDRESS = ""
				Me.DC = ""
			...
			For i As Integer = 0 To count - 1
				...
				If array(1) = "GOVERNMENT_STRUCTURE" Then
					If array(0).IndexOf(".") >= 1 Then
						...
						End Try
						Try
							text2 = Me.mvarCol_MemberOf2(text3) + "','" & array2(1)
